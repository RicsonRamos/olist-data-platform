...
def ingest_data():
    ...
    print(
        f"🚀 [INGESTION] Starting Incremental Load | {datetime.now().strftime('%H:%M:%S')}"
    )
    ...
    for file in files:
        ...
        try:
            ...
            df.to_parquet(
                os.path.join(SILVER_DIR, f"{table_name}.parquet"), index=False
            )

            # 4. Layer: Raw (PostgreSQL)
            df.to_sql(
                table_name, engine, schema=RAW_SCHEMA, if_exists="append", index=False
            )
            ...
            partition_dir = os.path.join(
                BRONZE_DIR, datetime.now().strftime("%Y-%m-%d")
            )
            os.makedirs(partition_dir, exist_ok=True)
            shutil.move(file_path, os.path.join(partition_dir, file))

            print(
                f"✅ [SUCCESS] {table_name} ({len(df)} rows) in {round(time.time() - start_time, 2)}s"
            )
