...
class DataQuality:
    ...
    def check_schema_drift(df, expected_columns, table_name):
        ...
        if drift:
            logger.warning(
                f"⚠️ Schema Drift detected in {table_name}: New columns {drift}"
            )
        if missing:
            logger.error(
                f"❌ Critical Schema Change in {table_name}: Missing columns {missing}"
            )
    ...
    def check_volume_anomaly(engine, table_name, current_count, threshold=0.5):
        ...
        try:
            with engine.connect() as conn:
                ...
                result = conn.execute(
                    text(
                        f"SELECT AVG(row_count) FROM metadata.processed_files WHERE file_name LIKE '{table_name}%'"
                    )
                ).scalar()
                ...
                if result and result > 0:
                    ...
                    if diff > threshold:
                        logger.warning(
                            f"📉 Volume Anomaly in {table_name}: Current {current_count} vs Avg {round(result, 0)} (Diff: {round(diff * 100, 2)}%)"
                        )
                        return False
