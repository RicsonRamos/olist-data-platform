class ReplayEngine:
    ...
    def replay_entity(self, table_name, date_from=None):
        ...
        history = self.bus.get_history(
            event_type="FILE_ARRIVED", partition_key=table_name
        )

        if date_from:
            history = [
                e for e in history if e["created_at"].strftime("%Y-%m-%d") >= date_from
            ]
        ...
        print(
            f"🚀 Selective Replay: Processing {len(history)} events for {table_name}..."
        )
        for event in history:
            # We emit with a new run_id (inherent in the processor) but keep the partition
            self.processor.process_event(
                "FILE_ARRIVED", payload=event["payload"], partition_key=table_name
            )
