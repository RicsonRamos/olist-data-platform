...
def test_check_volume_anomaly():
    ...
    # Within threshold
    assert DataQuality.check_volume_anomaly(mock_engine, "orders", 120, threshold=0.5) is True
    
    # Outside threshold
    assert DataQuality.check_volume_anomaly(mock_engine, "orders", 300, threshold=0.5) is False
